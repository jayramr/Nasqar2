tabItem(
    ...
    fluidRow(
        box(
            title = "Gene Expression Boxplot", solidHeader = T, status = "primary", width = 12,
            fluidRow(
                column(
                    6,
                    column(
                        ...
                        conditionalPanel(
                            ...
                            wellPanel(
                                radioButtons("box_plot_sel_gene_type", "Search by ",
                                    c(
                                        "gene.id" = "gene.id",
                                        "gene.name" = "gene.name"
                                    ),
    ...
    fluidRow(
        hr(),
        conditionalPanel(
            "output.boxplotComputed",
            tags$div(
                class = "BoxArea2",
                column(
                    12,
                    box(
                        title = "Boxplot", solidHeader = T, status = "primary", width = 12,
